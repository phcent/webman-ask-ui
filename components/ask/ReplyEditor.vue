<template>
  <div class="text">
    ...
    <div class="m-t-20">
      ...
      <template v-else>
        <el-button :loading="subLoading" type="primary" @click="submit">提交回复</el-button>
      </template>
    </div>
  </div>
</template>
...
<script>
import {urlUserAskReply} from '~/api/ask'
...
export default {
  ...
  methods:{
    ...
    submit(){
      ...
      this.$axios.post(urlUserAskReply+'/create',data).then(res=>{
        this.content=''
        this.editor.txt.html(this.content)
        this.$emit('change')
