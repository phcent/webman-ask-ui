<template>
  <div>
    ...
    >
      <template #prefix>
        <i :class="'ri-'+iconName" class="fz-20" style="color: #666"></i>
      </template>
      <template #default="{ item }">
          <i :class="'ri-'+item" class="fz-18" ></i>
          {{item}}
      </template>
    </el-autocomplete>
  </div>
  ...
  computed:{
    iconList(){
      ...
      if(!this.iconName){
        ...
        list = iconListType.filter(item=>{
          return item.indexOf(this.iconName) !== -1
        })
      }
      return list
    }
  ...
  methods:{
    querySearch(queryString, cb) {
      const results = queryString
        ? iconListType.filter(this.createFilter(queryString))
        : iconListType
      cb(results)
    },
