<template>
  <div>
    ...
    <div class="flex">
      <el-card shadow="never" :class="{'cate-show-card':visible}">
        <div >
          ...
          <!--    表格数据-->
          <!--操作按钮-->
          <div class="m-b-10 flex flex-col-between flex-row-center">
              <span class="flex">
                <el-button size="small" type="primary"
                           @click="$router.push('/manage/system/user/add')">
              ...
              <el-button size="small" type="primary" icon="ri-recycle-fill" @click="$router.push('/manage/system/user/recovery')">
                回收站</el-button>
              </span>
            <el-button type="primary" size="small" icon="ri-menu-line" @click="visible = !visible" circle style="float: right"></el-button>
          </div>
          <div class="bg-white">
            <el-table :data="tableData" :size="tableSize" style="width: 100%" v-loading="loading"  @selection-change="handleSelectionChange" class="sys-table" @sort-change="handleSort" >
              ...
              <el-table-column v-if="columnList.available_balance.show" prop="available_balance"  sortable :label="columnList.available_balance.label"
                               class-name="bg-color-1-danger color-danger" width="100" align="center"/>
              <el-table-column v-if="columnList.freeze_balance.show" prop="freeze_balance"  sortable :label="columnList.freeze_balance.label"
                               class-name="bg-color-1-primary color-primary" width="100" align="center"/>
              <el-table-column v-if="columnList.available_points.show" prop="available_points"  sortable :label="columnList.available_points.label"
                               class-name="bg-color-1-success color-success" width="100" align="center"/>
              <el-table-column v-if="columnList.freeze_points.show" prop="freeze_points"  sortable :label="columnList.freeze_points.label"
                               class-name="bg-color-1-primary color-primary" width="100" align="center"/>
              <el-table-column v-if="columnList.points.show" prop="points"  sortable :label="columnList.points.label"
                               class-name="bg-color-1-warning color-warning" width="100" align="center"/>
              <el-table-column v-if="columnList.login_num.show" prop="login_num"  sortable label="登入次数" width="100" align="center"/>
                ...
                <template slot-scope="{row}">
                  <el-dropdown trigger="click">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item >
                        <a href="javascript:void (0);" @click="$refs.UserPriceModal.open(row)">
                          <i class="ri-increase-decrease-line"></i>操作余额
                        </a>
                      </el-dropdown-item>
                      <el-dropdown-item >
                        <a href="javascript:void (0);" @click="$refs.UserPointsModal.open(row)">
                          <i class="ri-increase-decrease-line"></i>操作积分
                        </a>
                      </el-dropdown-item>
                      <el-dropdown-item >
                        <router-link :to="'/manage/system/user/update/'+row.id">
    ...
    <user-price-modal ref="UserPriceModal" @changeOk="getList"/>
    <user-points-modal ref="UserPointsModal" @changeOk="getList"/>
  </div>
</template>
...
import UserPriceModal from '~/components/system/UserPriceModal'
import UserPointsModal from '~/components/system/UserPointsModal'

export default {
  name: 'index',
  components: { UserPointsModal, UserPriceModal, UserPage },
  layout: "admin",
  head: {
  ...
  data(){
    return {
      ...
      columnList:{
        ...
        available_points:{label:'可用积分',show:true},
        freeze_points:{label:'冻结积分',show:true},
        points:{label:'累计积分',show:true},
  ...
  methods:{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        this.$axios.delete(urlAdminSysUser+'/destroy/'+ ids).then(res=>{
          this.$message.success('删除成功');
          this.getList();
        });
      })
