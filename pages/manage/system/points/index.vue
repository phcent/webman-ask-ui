<template>
  <div>
    <user-page title="积分管理" :total="total" :column-list="columnList"
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="tableData" :size="tableSize"  stripe class="blog-table"
        ...
        <el-table-column v-if="columnList.user_name.show" prop="user_name" :label="columnList.user_name.label" width="100" sortable align="center" >
        </el-table-column>
        <el-table-column v-if="columnList.available_points.show" prop="available_points" :label="columnList.available_points.label"
                         class-name="bg-color-1-success color-success" width="100" sortable align="center" />
        <el-table-column v-if="columnList.freeze_points.show" prop="freeze_points" :label="columnList.freeze_points.label"
                         class-name="bg-color-1-primary color-primary" width="100" sortable align="center" />
        <el-table-column v-if="columnList.old_available_points.show" prop="old_available_points" :label="columnList.old_available_points.label"
                         class-name="bg-color-1-success color-success" width="120" sortable align="center" />
        <el-table-column v-if="columnList.old_freeze_points.show" prop="old_freeze_points" :label="columnList.old_freeze_points.label"
        ...
        <el-table-column v-if="columnList.operate.show" :label="columnList.operate.label" width="80" align="center" fixed="right">
          <template slot-scope="{row}">
            <nuxt-link :to="'/manage/system/points/update/'+row.id">
...
// import { urlMangeReply } from '~/api/admin/system'
export default {
  name: 'index',
  components: { UserPage },
  layout: "admin",
  head: {
    titleTemplate: '积分日志 - 管理中心  - %s',
  },
  data(){
    return{
      loading:true,
      ...
      columnList:{
        ...
        old_available_points:{label:'原可用积分',show:true},
        old_freeze_points:{label:'原冻结积分',show:true},
