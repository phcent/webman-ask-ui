<template>
  <div>
    <user-page title="通知管理" :total="total" :column-list="columnList"
    ...
    <el-card shadow="never">
      ...
      <el-form v-if="showSearch" :inline="true" :model="queryForm" class="demo-form-inline">
        ...
        <el-form-item :label="columnList.from_user_id.label">
          <el-input v-model="queryForm.from_user_id" size="mini" :placeholder="'请输入'+columnList.from_user_id.label"></el-input>
        </el-form-item>
      ...
      <el-table v-loading="loading" :data="tableData" :size="tableSize"  stripe class="blog-table"
        ...
        <el-table-column v-if="columnList.from_user_id.show" prop="from_user_id" :label="columnList.from_user_id.label" width="100" sortable align="center" />
        <el-table-column v-if="columnList.content.show" prop="content" :label="columnList.content.label" min-width="100" sortable align="center" />
          ...
          <template slot-scope="{row}">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item >
                  <router-link :to="'/manage/ask/notice/update/'+row.id">
...
export default {
  ...
  data(){
    return{
      ...
      columnList:{
        ...
        from_user_id:{label:'操作会员',show:true},
        content:{label:'主题',show:true},
  ...
  methods:{
    ...
    getList(){
      this.loading=true
      this.$axios.get(urlAdminAskNotice+'/index',{params:this.queryForm}).then(res=>{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        this.$axios.delete(urlAdminAskNotice+'/destroy/'+ids).then(res=>{
