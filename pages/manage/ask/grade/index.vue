<template>
  <div>
    <user-page title="会员等级管理" :total="total" :column-list="columnList"
    ...
    <el-card shadow="never">
      ...
      <div class="m-b-10 flex">
        <el-button size="small" type="primary" icon="ri-add-fill"
                   @click="$router.push('/manage/ask/grade/add')">
          添加</el-button>
      ...
      <el-table v-loading="loading" :data="tableData" :size="tableSize"  stripe class="blog-table"
        ...
        <el-table-column v-if="columnList.name.show" prop="name" :label="columnList.name.label" min-width="100" sortable align="center" />
        <el-table-column v-if="columnList.image_name.show" prop="image_name" :label="columnList.image_name.label" min-width="100" sortable align="center" />
        <el-table-column v-if="columnList.points.show" prop="points" :label="columnList.points.label" width="100" sortable align="center" />
          ...
          <template slot-scope="{row}">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item >
                  <router-link :to="'/manage/ask/grade/update/'+row.id">
...
export default {
  ...
  data(){
    return{
      ...
      columnList:{
        ...
        name:{label:'等级名称',show:true},
        image_name:{label:'等级图片',show:true},
        points:{label:'所需积分',show:true},
  ...
  methods:{
    ...
    getList(){
      this.loading=true
      this.$axios.get(urlAdminAskGrade+'/index',{params:this.queryForm}).then(res=>{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        this.$axios.delete(urlAdminAskGrade+'/destroy/'+ids).then(res=>{
