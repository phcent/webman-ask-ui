<template>
  <div>
    <user-page title="文章回收站" :total="total" :column-list="columnList" :show-explain="false" :btn-list="['return']"
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="tableData" :size="tableSize"  stripe class="blog-table"
        ...
        <el-table-column v-if="columnList.title.show" prop="title" :label="columnList.title.label" min-width="100" sortable  :show-overflow-tooltip="true"/>
  ...
  methods:{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认销毁?销毁后不可恢复，请谨慎操作', '提示', {
        ...
        this.$axios.delete(urlAdminAskArticle+'/recovery/'+ids).then(res=>{
    ...
    handleBack(row){
      ...
      this.$axios.put(urlAdminAskArticle+'/recovery/'+ids).then(res=>{
