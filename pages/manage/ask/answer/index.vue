<template>
  <div>
    <user-page title="回答管理" :total="total" :column-list="columnList"
    ...
    <el-card shadow="never">
      ...
      <div class="m-b-10 flex">
        <el-button :disabled="selectList.length===0" size="small" icon="ri-delete-bin-5-fill" type="danger"
                   @click="handleDelete">
          删除</el-button>
        <el-button size="small" type="primary" icon="ri-recycle-fill" @click="$router.push('/manage/ask/answer/recovery')">
          回收站</el-button>
      </div>
      <!--表格-->
      <el-table v-loading="loading" :data="tableData" :size="tableSize"   class="blog-table"
        ...
        <el-table-column v-if="columnList.question_id.show" prop="question_id" :label="columnList.question_id.label" min-width="250" sortable  >
          <template slot-scope="{row}">
          ...
          <template slot-scope="{row}">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item >
                  <router-link :to="'/manage/ask/answer/'+row.id">
                    <i class="ri-eye-2-line"></i>查看
                  </router-link>
                </el-dropdown-item>
                <el-dropdown-item >
                  <router-link :to="'/manage/ask/answer/update/'+row.id">
  ...
  data(){
    return{
      ...
      columnList:{
        ...
        user_id:{label:'会员名称',show:true},
        question_id:{label:'问题',show:true},
  ...
  methods:{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        this.$axios.delete(urlAdminAskAnswer+'/destroy/'+ids).then(res=>{
    ...
    changeStatus(value,row){
      this.$axios.put(urlAdminAskAnswer+'/update/'+row.id,row).then(res=>{
