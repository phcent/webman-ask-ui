<template>
  <div>
    <user-page title="评论回收站" :total="total" :column-list="columnList" :show-explain="false" :btn-list="['return']"
  ...
  methods:{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认销毁?销毁后不可恢复，请谨慎操作', '提示', {
        ...
        this.$axios.delete(urlAdminAskReply+'/recovery/'+ids).then(res=>{
    ...
    handleBack(row){
      ...
      this.$axios.put(urlAdminAskReply+'/recovery/'+ids).then(res=>{
