<template>
  <div>
    <user-page title="问题管理" :total="total" :column-list="columnList"
    ...
    <el-card shadow="never">
      ...
      <el-form v-if="showSearch" :inline="true" :model="queryForm" class="demo-form-inline">
        ...
        <el-form-item :label="columnList.title.label">
          <el-input v-model="queryForm.title_like" size="mini" :placeholder="'请输入'+columnList.title.label"></el-input>
        </el-form-item>
        <el-form-item :label="columnList.cate_id.label">
          <el-select v-model="queryForm.cate_id"  size="mini" :placeholder="'请选择'+columnList.cate_id.label">
      ...
      <div class="m-b-10 flex">
        <el-button :disabled="selectList.length===0" size="small" icon="ri-delete-bin-5-fill" type="danger"
                   @click="handleDelete">
          删除</el-button>
        <el-button size="small" type="primary" icon="ri-recycle-fill" @click="$router.push('/manage/ask/question/recovery')">
          回收站</el-button>
      </div>
      <!--表格-->
      <el-table v-loading="loading" :data="tableData" :size="tableSize"  stripe class="blog-table"
        ...
        <el-table-column v-if="columnList.cate_id.show" prop="cate_id" :label="columnList.cate_id.label" width="110" sortable  >
        ...
        <el-table-column v-if="columnList.reward_balance.show" prop="reward_balance" :label="columnList.reward_balance.label" width="110"
                         class-name="bg-color-1-danger color-danger" sortable align="center" >
          <template slot-scope="{row}">
            ￥{{row.reward_balance}}
          </template>
        </el-table-column>
        <el-table-column v-if="columnList.reward_points.show" prop="reward_points" :label="columnList.reward_points.label" width="110"
                         class-name="bg-color-1-warning color-warning" sortable align="center" />
        <el-table-column v-if="columnList.digg_num.show" prop="digg_num" :label="columnList.digg_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.step_num.show" prop="step_num" :label="columnList.step_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.view_num.show" prop="view_num" :label="columnList.view_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.follow_num.show" prop="follow_num" :label="columnList.follow_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.report_num.show" prop="report_num" :label="columnList.report_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.collection_num.show" prop="collection_num" :label="columnList.collection_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.thank_num.show" prop="thank_num" :label="columnList.thank_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.comment_num.show" prop="comment_num" :label="columnList.comment_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.reply_num.show" prop="reply_num" :label="columnList.reply_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.best_comment.show" prop="best_comment" :label="columnList.best_comment.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.share_num.show" prop="share_num" :label="columnList.share_num.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.hot_sort.show" prop="hot_sort" :label="columnList.hot_sort.label" width="110" sortable align="center" />
        <el-table-column v-if="columnList.top_sort.show" prop="top_sort" :label="columnList.top_sort.label" width="110" sortable align="center" />
          ...
          <template slot-scope="{row}">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item >
                  <router-link :to="'/manage/ask/question/update/'+row.id">
  ...
  methods:{
    ...
    handleDelete(row){
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        this.$axios.delete(urlAdminAskQuestion+'/destroy/'+ids).then(res=>{
    ...
    changeStatus(value,row){
      this.$axios.put(urlAdminAskQuestion+'/update/'+row.id,row).then(res=>{
